 ...
 * 定期发布里程计（odom）和真值（truth），并可选发布3D网格模型标记。
 */
class TargetUavNode
{
public:
...
private:
  ...
  void loadParams()
  {
    ...
    pnh_.param("target_uav/marker_color_r", marker_color_r_, 1.0);
    pnh_.param("target_uav/marker_color_g", marker_color_g_, 0.05);
    pnh_.param("target_uav/marker_color_b", marker_color_b_, 0.02);
    pnh_.param("target_uav/marker_color_a", marker_color_a_, 1.0);
  }

  /**
   * @brief 获取当前节点的命名空间中的无人机名称
   * @return 例如 "/target_0" 返回 "target_0"，根命名空间返回空字符串
   */
   ...
   *
   * 参数格式：target_uav/initial_states = { target_0: [x, y, z, yaw], ... }
   * 可以覆盖通过普通参数设置的初始值。
   */
  void loadNamespacedInitialState()
  {
    ...
    if (name.empty() || !pnh_.getParam("target_uav/initial_states", initial_states))
    {
      return;
    }
...
private:
  ...
  double marker_color_r_ = 1.0;
  double marker_color_g_ = 0.05;
  double marker_color_b_ = 0.02;
...
/**
 * @brief 主函数：初始化节点，创建目标无人机节点实例，进入ROS事件循环
 */
